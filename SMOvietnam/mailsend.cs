...
namespace SMOvietnam
{
    public partial class mailsend : Form
    {
        ...
        public mailsend(main mainForm)
        {
            ...
            sqlite_cmd.CommandText = "SELECT * FROM email_campaign WHERE type_save = '2' ORDER BY id DESC";
            sqlite_datareader = sqlite_cmd.ExecuteReader();

            int _i_mail = 0;
            ListMailsendok.Rows.Clear();
            while (sqlite_datareader.Read()) // Read() returns true if there is still a result line to read
            {
                ListMailsendok.Rows.Add();
                ListMailsendok.Rows[_i_mail].Cells[0].Value = (_i_mail + 1);
                ListMailsendok.Rows[_i_mail].Cells[1].Value = sqlite_datareader["email_name"].ToString();
                ListMailsendok.Rows[_i_mail].Cells[4].Value = sqlite_datareader["id"].ToString();
                _i_mail++;
            }
            sqlite_conn.Close();
        ...
        private void ListMailsendok_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                int _id = 0;
                _id = Convert.ToInt32(ListMailsendok.Rows[e.RowIndex].Cells[4].Value);
                if (e.ColumnIndex == 2)
                {
                    try
                    {
                        ...
                        while (sqlite_datareader.Read()) // Read() returns true if there is still a result line to read
                        {
                            ...
                            mainForm.dgvEmailListSend.Rows[_i_mail].Cells[1].Value = (_i_mail + 1);
                            mainForm.dgvEmailListSend.Rows[_i_mail].Cells[0].Value = true;
                            mainForm.dgvEmailListSend.Rows[_i_mail].Cells[2].Value = sqlite_datareader["email_name"].ToString();
                            mainForm.dgvEmailListSend.Rows[_i_mail].Cells[3].Value = sqlite_datareader["fullname"].ToString();
                            ...
                            if (Convert.ToInt32(sqlite_datareader["type_mail"]) != 1)
                            {
                                ...
                                mainForm.dgvEmailListSend.Rows[_i_mail].Cells[4].Style.BackColor = System.Drawing.Color.Green;
                                mainForm.dgvEmailListSend.Rows[_i_mail].Cells[4].Style.ForeColor = System.Drawing.Color.White;

                            }

                            mainForm.dgvEmailListSend.Rows[_i_mail].Cells[4].Value = lbl_status_send;
                            _i_mail++;
                        }
                        sqlite_conn.Close();
                ...
                else if (e.ColumnIndex == 3)
                {
                    if (MessageBox.Show("Bạn có chắc chắn muốn xóa dòng đã chọn", "Xác nhận", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        while (sqlite_datareader.Read()) // Read() returns true if there is still a result line to read
                        {
                            ListMailsendok.Rows.Add();
                            ListMailsendok.Rows[_i_mail].Cells[0].Value = (_i_mail + 1);
                            ListMailsendok.Rows[_i_mail].Cells[2].Value = sqlite_datareader["email_name"].ToString();
                            ListMailsendok.Rows[_i_mail].Cells[1].Value = (_i_mail + 1);
                            _i_mail++;
                        }
                        sqlite_conn.Close();
                    }
